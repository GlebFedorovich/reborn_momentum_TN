...
# Pin BLAS to 1 thread/process regardless of launch command — see header.
...
folder_name = "grouped_ansatz/output/phase_scan_mpi/chi_scan"
...
# ---------------------------------------------------------------------------
# Building blocks — identical to chi_scan_mpi.jl / phase_scan_vmc.jl.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Final summary table (root only).
# ---------------------------------------------------------------------------
if IS_ROOT
    txtfile = folder_name * "/phase_scan_summary_L$(L).txt"
    open(txtfile, "w") do io
        @printf(io, "phase_scan_vmc_mpi summary — L=%d, %d rank(s) (%d replica(s)/step)\n\n",
                L, N_RANKS, max(N_RANKS - 1, 1))
    ...
    println("\nwrote $txtfile")
    println("\nphase_scan_vmc_mpi done. Files in $folder_name")
end
